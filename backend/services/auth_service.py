...
Handles user sessions, JWT tokens, and query counting
"""
...
class AuthService:
    def __init__(self, db_path: str = None, jwt_secret: str = None):
        ...
        if db_path is None:
            db_path = os.getenv("DATABASE_PATH", "/tmp/auth.db" if os.getenv("RAILWAY_ENVIRONMENT") else "auth.db")
        self.db_path = db_path
        self.jwt_secret = jwt_secret or os.getenv("JWT_SECRET", "fallback-secret-change-in-production")
        self.google_client_id = "910315304252-im8oclg36n7dun7hjs2atkv8p2ln7ng7.apps.googleusercontent.com"
        self.init_database()
